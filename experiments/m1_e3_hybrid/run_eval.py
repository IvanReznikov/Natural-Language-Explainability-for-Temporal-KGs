    ...
    def hybrid_generate(self, fact: TemporalFact, fact_type: str) -> HybridResult:
        ...
        try:
            ...
            sorted_templates = sorted(
                renders.keys(), key=lambda k: renders[k]["score"], reverse=True
            )
            ...
            should_polish = self.enable_polish and (
                (
            ...
            if should_polish:
                ...
                if polished:
                    ...
                    if polished_flesch > result_flesch and not self._detect_hallucination(
                        polished, fact
                    ):
            ...
            return HybridResult(
                ...
                render_details=self._render_details_with_justification(
                    best_template_id, renders, result_output, fact
                ),
...
Rewritten:"""
            ...
            response = chain.invoke({"output": output, "fact_str": str(fact)})
    ...
    def _render_details_with_justification(
        self, template_id: str, renders: Dict[str, Dict[str, Any]], output: str, fact: TemporalFact
    ) -> str:
        ...
        belief = Belief(
            belief_id=belief_id,
            payload={"fact_type": fact.fact_type, "content": getattr(fact, "content", {})},
        )
        ...
        renders_copy[template_id] = {
            **renders_copy.get(template_id, {}),
    ...
    def _compute_metrics(self) -> HybridMetrics:
        ...
        valid_results = [r for r in self.all_results if r.output and r.words > 0]
        ...
        flesch_std = (
            sum((f - flesch_mean) ** 2 for f in flesch_scores) / len(flesch_scores)
        ) ** 0.5
        ...
        type_coverage = (
            sum(1 for f in type_flesch.values() if f > 70) / len(type_flesch) if type_flesch else 0
        )
        ...
        hallucination_rate = (
            sum(1 for r in valid_results if r.hallucination_detected) / len(valid_results)
            if valid_results
            else 0
        )
...
def main():
    ...
    parser.add_argument(
        "--polish", action=argparse.BooleanOptionalAction, default=True, help="Enable polishing"
    )
    parser.add_argument(
        "--anchor-snippets",
        type=int,
    ...
    parser.add_argument(
        "--no-justification",
        dest="enable_justification",
        action="store_false",
        help="Disable justification plumbing",
    )
