...
def analyze_results(results_dir: str):
    ...
    print("\n📊 SUMMARY METRICS")
    print("-"*100)
    print(f"Flesch Score:       {metrics.get('flesch_mean', 0):.1f} ± {metrics.get('flesch_std', 0):.1f}")
    ...
    if csv_path.exists():
        ...
        print("-"*100)
        print(f"{'Type':<15} {'Template':<25} {'Source':<12} {'Flesch':<8} {'Words':<6} {'Output':<40}")
        print("-"*100)
        ...
        for typ in sorted(types.keys()):
            ...
            print(f"{typ:<20} Flesch: {avg_flesch:6.1f}  Words: {avg_words:5.1f}  Sources: {source_str}")
        ...
        for source in sorted(sources.keys()):
            ...
            print(f"{source:<15} {data['count']:4} renders ({pct:5.1f}%)  Avg Flesch: {avg_flesch:6.1f}")
