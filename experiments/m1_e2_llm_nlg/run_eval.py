...
→ """,
            ...
            chat_prompt = ChatPromptTemplate.from_messages([
                ("system", """Output ONLY 1 short natural sentence (8-12 words).
Use EXACT fact data. No explanations. No JSON. No periods at start."""),
    ...
    def evaluate(self) -> M1E1EvaluationReportV3:
        ...
        for fact_type_str, facts in examples.items():
            ...
            for i, fact in enumerate(facts[:self.n_examples]):  # Safety cap
                ...
                if (i + 1) % 10 == 0:
                    recent_avg = sum(r['flesch_score'] for r in all_renders[-10:] if r['success']) / max(1, sum(1 for r in all_renders[-10:] if r['success']))
            ...
            console.print(f"{fact_type_str}: {type_success}/{len(facts)} ({type_avg:.0f} Flesch, {success_rate:.0%} success)")
        ...
        console.print(Panel.fit(
            ...
            title="M1-E2 PRODUCTION RESULTS", style=color
        ))
    ...
    def _build_report(self, renders: List[Dict]) -> M1E1EvaluationReportV3:
        ...
        if valid_renders:
            report.overall_flesch = sum(r['flesch_score'] for r in valid_renders) / len(valid_renders)
            report.overall_information_density = sum(r['information_density'] for r in valid_renders) / len(valid_renders)
        ...
        for fact_type in set(r['template_type'] for r in renders):
            ...
            avg_density = sum(r['information_density'] for r in type_valid) / max(1, len(type_valid))
