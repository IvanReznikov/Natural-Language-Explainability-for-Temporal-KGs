...
def _sample_stratified(
    rows: Sequence[dict], n: int, seed: int, stratify_domain: bool
) -> List[dict]:
    if n <= 0:
        return []
...
def main() -> None:
    ...
    ap.add_argument(
        ...
        type=str,
        default=None,
        help="Optional predictions JSONL with {id, prediction}",
    )
    ...
    ap.add_argument(
        "--no-domain-stratify", action="store_true", help="Disable domain stratification"
    )
    ...
    if args.human_loop == "human":
        human_export_path = (
            Path(args.human_loop_export)
            if args.human_loop_export
            else (out_dir / "m3_e2_human_tasks.jsonl")
        )
    ...
    if args.human_loop == "llm":
        llm = M3E2HumanLoopLLMScorer(
            model=args.llm_model, temperature=args.llm_temperature, max_tokens=args.llm_max_tokens
        )
        ...
        for row in per_item_rows:
            ...
            extra = llm.score(
                record=record, prediction_text=pred_text, bucket=str(row.get("bucket") or "")
            )
            if extra:
                for k in [
                    "ambiguity_resolution",
