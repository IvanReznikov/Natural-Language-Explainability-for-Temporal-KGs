...
class AccuracyEvaluator:
    ...
    def evaluate_output(self, fact: TemporalFact, output: str, fact_type: str, 
                       template_id: str, source: str) -> AccuracyResult:
        """
    ...
    def _compute_metrics(self) -> AccuracyMetrics:
        ...
        relation_mean = sum(r.relation_preservation for r in self.all_results) / len(self.all_results)
        ...
        template_acc = sum(r.overall_accuracy for r in template_results) / len(template_results) if template_results else 0
        polished_acc = sum(r.overall_accuracy for r in polished_results) / len(polished_results) if polished_results else 0
    ...
    def _save_results(self, metrics: AccuracyMetrics, source_dir: str):
        ...
        results_dir = Path(__file__).resolve().parents[2] / "output" / "m1_e4_accuracy" / uuid4().hex
        ...
        with open(csv_path, "w", newline="") as f:
            writer = csv.DictWriter(
                f,
                fieldnames=["type", "source", "overall_accuracy", "date", "entity", "relation", 
                           "hallucinated", "hallucination_type", "output"]
            )
            writer.writeheader()
