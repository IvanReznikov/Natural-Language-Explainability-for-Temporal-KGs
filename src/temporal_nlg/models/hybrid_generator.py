...
class HybridGenerator:
    ...
    def generate(
        self, fact: TemporalFact, force_strategy: Optional[str] = None
    ) -> GenerationResult:
        """
    ...
    def _generate_template(self, fact: TemporalFact, cache_key: str) -> GenerationResult:
    ...
    def _generate_polished(self, fact: TemporalFact, cache_key: str) -> GenerationResult:
...
Output ONLY the improved version (one sentence)."""
    ...
    def _generate_llm(self, fact: TemporalFact) -> GenerationResult:
        ...
        return GenerationResult(text=text, strategy="llm", confidence=0.7)
    ...
    def batch_generate(
        self, facts: List[TemporalFact], show_progress: bool = False
        ...
        for fact in iterator:
            try:
                ...
                results.append(
                    GenerationResult(text=f"[ERROR: {str(e)}]", strategy="error", confidence=0.0)
                )
