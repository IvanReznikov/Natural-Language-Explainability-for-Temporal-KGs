...
class M1E1EvaluationReportV3:
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'type_metrics': {
                k: asdict(v) for k, v in self.type_metrics.items()
            },
...
class M1E1EvaluatorV3:
    ...
    def evaluate_all_templates(
        ...
        # Initialize report
        self.report = M1E1EvaluationReportV3(
        ...
        for fact_type, library in libraries.items():
            ...
            avg_flesch = sum(type_flesch_scores) / len(type_flesch_scores) if type_flesch_scores else 0.0
            avg_clarity = sum(type_clarity_scores) / len(type_clarity_scores) if type_clarity_scores else 0.0
            avg_info_density = sum(type_information_densities) / len(type_information_densities) if type_information_densities else 0.0
        ...
        self.report.overall_coverage = (
            self.report.successful_renders / max(1, self.report.total_renders)
        )
        ...
        self.report.overall_information_density = (
            sum(information_densities) / len(information_densities) if information_densities else 0.0
        )
        self.report.type_coverage = (
            types_passing_flesch / len(libraries) if libraries else 0.0
        )
    ...
    def generate_text_report(self) -> str:
        ...
        lines.append(f"Overall Success Rate: {self.report.overall_success_rate * 100:.1f}% (target >95%)")
        lines.append(f"Overall Information Density: {self.report.overall_information_density:.3f} (target >0.5)")
        lines.append(f"Type Coverage (% passing Flesch >45): {self.report.type_coverage * 100:.1f}% (target >80%)")
        lines.append("")
        ...
        lines.append("-" * 100)
        lines.append(f"{'✓' if self.report.overall_coverage > 0.85 else '✗'} Coverage >85%: {'PASS' if self.report.overall_coverage > 0.85 else 'FAIL'} ({self.report.overall_coverage * 100:.1f}%)")
        lines.append(f"{'✓' if self.report.overall_flesch > 45 else '✗'} Flesch >45: {'PASS' if self.report.overall_flesch > 45 else 'FAIL'} ({self.report.overall_flesch:.1f})")
        lines.append(f"{'✓' if self.report.overall_clarity > 4.0 else '✗'} Clarity >4.0: {'PASS' if self.report.overall_clarity > 4.0 else 'FAIL'} ({self.report.overall_clarity:.2f})")
        lines.append(f"{'✓' if self.report.overall_success_rate > 0.95 else '✗'} Success Rate >95%: {'PASS' if self.report.overall_success_rate > 0.95 else 'FAIL'} ({self.report.overall_success_rate * 100:.1f}%)")
        lines.append(f"{'✓' if self.report.overall_information_density > 0.5 else '✗'} Info Density >0.5: {'PASS' if self.report.overall_information_density > 0.5 else 'FAIL'} ({self.report.overall_information_density:.3f})")
        lines.append(f"{'✓' if self.report.type_coverage > 0.8 else '✗'} Type Coverage >80%: {'PASS' if self.report.type_coverage > 0.8 else 'FAIL'} ({self.report.type_coverage * 100:.1f}%)")
        lines.append("")
...
def evaluate_and_save(
    output_dir: str = "experiments/m1_e1_templates/results",
    n_examples: int = 100
    ...
    report = evaluator.evaluate_all_templates(
        n_examples_per_type=n_examples,
        save_to_file=str(json_path)
    )
