...
class AccuracyEvaluator:
    ...
    def evaluate(self, fact: Any, generated_text: str) -> AccuracyMetrics:
        """
        ...
        overall = 0.4 * date_score + 0.3 * entity_score + 0.3 * relation_score
    ...
    def batch_evaluate(self, facts: List[Any], texts: List[str]) -> List[AccuracyMetrics]:
        """
        ...
        return [self.evaluate(fact, text) for fact, text in zip(facts, texts)]

    def aggregate_metrics(self, metrics: List[AccuracyMetrics]) -> Dict[str, float]:
        """
        ...
        return {
            ...
            "mean_relation_preservation": sum(m.relation_preservation for m in metrics)
            / len(metrics),
